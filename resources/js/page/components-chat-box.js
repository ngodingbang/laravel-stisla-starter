...
for(var i = 0; i < chats.length; i++) {
  ...
  $.chatCtrl('#mychatbox', {
    ...
    picture: (chats[i].position == 'left' ? assetUrl + '/img/avatar/avatar-1.png' : assetUrl + '/img/avatar/avatar-2.png'),
...
$("#chat-form").submit(function() {
  ...
  if(me.find('input').val().trim().length > 0) {
    $.chatCtrl('#mychatbox', {
      ...
      picture: assetUrl + '/img/avatar/avatar-2.png',
    });
    me.find('input').val('');
  }
  return false;
...
for(var i = 0; i < chats.length; i++) {
  ...
  $.chatCtrl('#mychatbox2', {
    text: (chats[i].text != undefined ? chats[i].text : ''),
    picture: (chats[i].position == 'left' ? assetUrl + '/img/avatar/avatar-5.png' : assetUrl + '/img/avatar/avatar-2.png'),
    position: 'chat-'+chats[i].position,
    type: type
  });
}
